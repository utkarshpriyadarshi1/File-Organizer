...
This directory contains utility scripts to automate production compilation and code-signing of the **e-abhilekh** desktop application deliverables using a self-signed developer certificate.

## Scripts Directory

*   **`generate-cert.ps1`**: A PowerShell script that generates a self-signed code signing certificate and exports it to `e-abhilekh-cert.pfx`.
...
1.  Right-click `e-abhilekh-cert.pfx` and select **Install PFX** (or double-click the file).
2.  Select **Local Machine** as the Store Location and click Next.
3.  Confirm the file path and click Next.
4.  Enter the password: **`Abhilekh2026!`** and click Next.
...
```cmd
signtool verify /pa /v backend\target\e-abhilekh-0.0.3.jar
```
