...
const dirsToClean = [
    path.join(rootDir, 'server', 'target'),
    path.join(rootDir, 'ui', 'build'),
...
const filesToClean = [
    ...
    path.join(rootDir, 'server', 'fboss.db'),
    path.join(rootDir, 'server', 'e-abhilekh.db')
];
...
// 3. Clean JVM crash logs in root and server
const locationsToScanLogs = [rootDir, path.join(rootDir, 'server')];
