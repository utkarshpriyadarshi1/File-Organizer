...
│   └── e-abhilekh_schema.sql # SQLite database schema definitions
```
...
Build and run the Java backend using your IDE or Maven. It runs fully local out-of-the-box using an in-memory task queue/cache and the SQLite file-based database driver (creating a local `e-abhilekh.db` file in the working directory).
