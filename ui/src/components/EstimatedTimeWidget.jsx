...
const EstimatedTimeWidget = ({ folderPath, operationType = "ORGANIZE" }) => {
    ...
    const calculateEstimation = (totalFiles, totalSize) => {
        ...
        if (operationType === "DUPLICATES") {
            msPerFile = 50; // slower due to hashing
            msPerGB = 2000;
        }

        const sizeInGB = totalSize / (1024 * 1024 * 1024);
        const estimatedMs = (totalFiles * msPerFile) + (sizeInGB * msPerGB);
